...
test('updates documents matching a filter from the config page', async ({
  page,
  login,
  tsClient,
  seededCollection,
  ...
  await page.getByLabel(/update filter/i).fill('brand:=SeatCo');
  await page.getByLabel(/document patch/i).fill('{ "price": 99.0 }');
