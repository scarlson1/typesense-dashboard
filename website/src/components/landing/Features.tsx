    ...
    <div className='wrap'>
      ...
      >
        {features.map((f) => (
          <Card
            variant='outlined'
            className={`reveal${f.delay ? ` ${f.delay}` : ''}`}
            key={f.title}
            sx={[
              {
                borderRadius: 1.5,
                '&:hover': {
                  borderColor: (theme) =>
                    theme.vars.palette.design.accentBorder,
                  boxShadow:
                    '0 1px 2px rgba(15,40,80,0.04), 0 18px 40px -22px rgba(15,40,80,0.18)',
                },
              },
              (theme) =>
                theme.applyStyles('dark', {
                  '&:hover': {
                    boxShadow:
                      '0 1px 2px rgba(0, 0, 0, 0.4), 0 16px 40px -16px rgba(0, 0, 0, 0.6)',
                  },
                }),
            ]}
          >
            <CardContent>
              <Stack
                direction='row'
                spacing={2}
                sx={{ mb: { xs: 1.5, sm: 2, lg: 2.5 }, alignItems: 'center' }}
              >
                <Box
                  sx={{
                    width: 40,
                    height: 40,
                    background: (theme) => theme.vars.palette.design.accentSoft,
                    color: (theme) => theme.vars.palette.design.accent,
                    mb: 2,
                    border: (theme) =>
                      `1px solid ${theme.vars.palette.design.accentBorder}`,
                    borderRadius: 1,
                    display: 'flex',
        ...
        {/* {features.map((f) => (
          <div
            className={`feature reveal${f.delay ? ` ${f.delay}` : ''}`}
            key={f.title}
          >
